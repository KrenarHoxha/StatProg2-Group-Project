...
In the exploratory model, `unemployment`, `female_non_agri`, and `avg_weekly_hours` all have negative coefficients, conditional on the other included variables. The negative coefficients for `unemployment` and `avg_weekly_hours` are consistent with the pairwise correlations reported above, while the coefficient for `female_non_agri` is more negative than its weak pairwise correlation. Differences between correlation and regression patterns can occur because the model conditions on income group, subsample, and the other labour market indicators. The estimates describe conditional associations only and should not be interpreted causally.
